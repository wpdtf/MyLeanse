...
public class TakeOffCallbackHandler(ILogger<TakeOffCallbackHandler> logger, MessageSendAsync sendAsync, LeanseStorage leanseStorage) : ICallbackHandler
{
    public string Action => "takeOff";
    private readonly ILogger<TakeOffCallbackHandler> _logger = logger;
    ...
    public async Task HandleAsync(CallbackQuery query, string[] args, CancellationToken ct)
    {
        ...
        {
            _leanseStorage.End(query.From.Id);
            text = "Линзы сняты!" + GetTextMessage(info);
        }
        else
            text = "Отсутствовали надетые линзы!" + GetTextMessage(info);
